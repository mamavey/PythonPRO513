...
def init_db(database_name, currencies: list[tuple[str, str]]):
    conn = sqlite3.connect(fr'databases\{database_name}.db')
    cursor = conn.cursor()
...
def save_rates_to_db(database_name, rates):
    if 'error' in rates:
        return False
    conn = sqlite3.connect(fr'databases\{database_name}.db')
    cursor = conn.cursor()
...
def create_indexes(database_name):
    conn = sqlite3.connect(fr'databases\{database_name}.db')
    cursor = conn.cursor()
    try:
